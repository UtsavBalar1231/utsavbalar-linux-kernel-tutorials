...
static ssize_t sync_demo_write(struct file *, const char __user *, size_t, loff_t *);
...
static int sync_proc_show(struct seq_file *m, void *v)
{
    ...
    rwsem_value = counter_values[0]; /* Using the first counter for rwsem demo */
...
static int demo_thread_fn(void *data)
{
    ...
    while (!kthread_should_stop() && !thread_should_stop) {
        ...
        counter_values[0] = atomic_read(&atomic_counter); /* Sync with atomic */
                              ...
                              size_t count, loff_t *offset)
{
    ...
    len = snprintf(device_buffer, BUFFER_SIZE, 
                  "Atomic counter: %d\nSpinlock counter: %d\nMutex counter: %d\nSemaphore counter: %d\n",
                  atomic_read(&atomic_counter), counter_values[1], counter_values[2], counter_values[3]);
...
static ssize_t sync_demo_write(struct file *file, const char __user *user_buffer, 
                               size_t count, loff_t *offset)
{
    char buffer[16];
...
static int __init sync_demo_init(void)
{
    ...
    /* Create the device */
    sync_device = device_create(sync_class, NULL, MKDEV(major_number, 0), NULL, DEVICE_NAME);
