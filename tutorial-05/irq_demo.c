...
static ssize_t irq_demo_write(struct file *, const char __user *, size_t,
			      loff_t *);
...
static int irq_proc_show(struct seq_file *m, void *v)
{
	...
	if (using_timer) {
		seq_printf(
			m,
			"Using timer to simulate interrupts (period: %ld ns)\n",
			TIMER_PERIOD_NS);
	}
	...
	if (bh_time) {
		...
		if (irq_time) {
			...
			s64 latency_ns =
				ktime_to_ns(ktime_sub(bh_time, irq_time));
...
static void demo_delayed_work_handler(struct work_struct *work)
{
	...
	queue_delayed_work(demo_wq, &delayed_work,
			     ...
			     size_t count, loff_t *offset)
{
	...
	len = snprintf(
		...
		atomic_read(&irq_count), atomic_read(&bottom_half_count),
		atomic_read(&delayed_work_count));
...
static int __init irq_demo_init(void)
{
	...
	if (using_timer) {
		...
		hrtimer_start(&demo_timer, ns_to_ktime(TIMER_PERIOD_NS),
			      HRTIMER_MODE_REL);
	}
	...
	/* Create the device */
	irq_device = device_create(irq_class, NULL, MKDEV(major_number, 0),
				   NULL, DEVICE_NAME);
